...
- (NSNumber *)averageRSSI {
    return [_entry.RSSIMeasurementTable averageRSSIMeasurementInPastTimeInterval:SILRetailBeaconAppDistanceInterval];
}

- (SILBeaconProximity)proximity {
    return _entry.beacon.beacon ?
    [SILProximityCalculator proximityWithBeacon:_entry.beacon.beacon] :
    [SILProximityCalculator estimatedProximityWithRSSI:[self averageRSSI]
                                      calibrationPower:@(_entry.beacon.calibrationPower)];
}
...
- (NSString *)formattedRSSI {
    int RSSI = [[self averageRSSI] intValue];
